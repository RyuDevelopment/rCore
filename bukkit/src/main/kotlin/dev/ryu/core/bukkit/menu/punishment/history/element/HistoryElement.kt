...
class HistoryElement(private val punishment: Punishment, private val profile: Profile, val parent: Menu) : Button() {
    ...
    override fun getName(player: Player): String {
        ...
        return "${ChatColor.GOLD}${TimeUtil.formatIntoCalendarString(Date(this.punishment.created))} ${if (this.punishment.isIP() && this.punishment.type != Punishment.Type.BLACKLIST) "${ChatColor.RED}[IP]" else ""}"
    }
    ...
    override fun getDescription(player: Player): MutableList<String> {
        ...
        lore.add("")
        lore.add(" ${ChatColor.WHITE}${UnicodeUtil.VERTICAL_LINE} ${ChatColor.YELLOW}By${ChatColor.GRAY}: ${ChatColor.WHITE}${ if (this.punishment.sender == UUID.fromString(Profile.CONSOLE_UUID)) "${ChatColor.DARK_RED}${ChatColor.BOLD}Console" else ProfileModule.findById(this.punishment.sender)!!.name }")
        ...
        if (this.canPardon) {
            lore.add("")
            lore.add("${ChatColor.RED}Click to pardon.")
        }
    ...
    override fun getDamageValue(player: Player): Byte {
        ...
        return if (this.punishment.type == Punishment.Type.KICK || this.punishment.type == Punishment.Type.WARN) 0 else dyeColor.woolData
    }
