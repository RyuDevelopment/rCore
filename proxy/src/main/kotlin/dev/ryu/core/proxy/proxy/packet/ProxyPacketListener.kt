...
class ProxyPacketListener(private val instance: Core) : OrbitListener {
    ...
    fun onSend(data: JsonObject) {
        ...
        val players = dev.ryu.core.shared.CoreAPI.getGson().fromJson<List<String>>(data["players"].asJsonPrimitive.asString,List::class.java).map{UUID.fromString(it)}.toList()
